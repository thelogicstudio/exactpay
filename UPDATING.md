...
2. Make the following change to correct it:
   ...
   6. In `Document.type` the response type is listed as "string", but also references a field like `"$ref": "#/components/schemas/DocumentUploadType"`. Remove the whole "allOf" definition from this.
...
5. Run `openapi-generator-cli generate -i openapi.json -g php -o ./client -c config.json`.
